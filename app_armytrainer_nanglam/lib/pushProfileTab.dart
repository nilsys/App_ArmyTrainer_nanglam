...
class _PushProfile extends State<PushProfile> {
  ...
  Future<List<Push>> _loadPush(DateTime _date) async {
    List<String> list = List<String>();
    List<Push> pushList = List<Push>();
    _max = 0;
    if (_now == _date) {
      for (var i = -6; i < 1; i++) {
        list.add(
      ...
      await Future.forEach(list, (element) async {
        Push push = (await DBHelper().getPush(element) ?? null);
        if (push != null) {
          pushList.add(push);
          if (_max < push.countLevel + push.countRecord) {
            _max = push.countLevel + push.countRecord;
          }
        } else {
          pushList.add(Push(countLevel: 0, countRecord: 0, date: element));
        }
      });
      return pushList;
    } else {
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          ...
          FutureBuilder(
            future: _loadPush(_date),
            builder:
                (BuildContext context, AsyncSnapshot<List<Push>> snapshot) {
              if (snapshot.hasData) {
                return Center(
                  child: Stack(
                    children: [
